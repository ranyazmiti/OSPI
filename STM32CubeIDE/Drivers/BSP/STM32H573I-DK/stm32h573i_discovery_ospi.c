      ...
      if (OspiNor_IsMspCbValid[Instance] == 0UL)
      {
        if (BSP_OSPI_NOR_RegisterDefaultMspCallbacks(Instance) != BSP_ERROR_NONE)
        {
          return BSP_ERROR_PERIPH_FAILURE;
			...
			ospi_init.MemorySize = (uint32_t) POSITION_VAL(
					(uint32_t )pInfo.FlashSize);
			...
			if (MX_OSPI_NOR_Init(&hospi_nor[Instance], &ospi_init) != HAL_OK) {
				...
				else if (MX25LM51245G_AutoPollingMemReady(&hospi_nor[Instance],
						Ospi_Nor_Ctx[Instance].InterfaceMode,
						Ospi_Nor_Ctx[Instance].TransferRate) != MX25LM51245G_OK) {
					ret = BSP_ERROR_COMPONENT_FAILURE;
				}/* Configure the memory */
				else if (BSP_OSPI_NOR_ConfigFlash(Instance, Init->InterfaceMode,
	...
	if (Instance >= OSPI_NOR_INSTANCES_NUMBER) {
		...
		if (Ospi_Nor_Ctx[Instance].IsInitialized != OSPI_ACCESS_NONE) {
			...
			if (Ospi_Nor_Ctx[Instance].IsInitialized == OSPI_ACCESS_MMP) {
				if (BSP_OSPI_NOR_DisableMemoryMappedMode(
						Instance) != BSP_ERROR_NONE) {
 ...
 * @retval BSP status
 */
__weak HAL_StatusTypeDef MX_OSPI_NOR_Init(XSPI_HandleTypeDef *hospi,
		MX_OSPI_InitTypeDef *Init) {
 ...
 * @retval BSP status
 */
int32_t BSP_OSPI_NOR_Read(uint32_t Instance, uint8_t *pData, uint32_t ReadAddr,
		uint32_t Size) {
	int32_t ret;
	...
	if (Instance >= OSPI_NOR_INSTANCES_NUMBER) {
		...
		if (Ospi_Nor_Ctx[Instance].TransferRate == BSP_OSPI_NOR_STR_TRANSFER) {
			if (MX25LM51245G_ReadSTR(&hospi_nor[Instance],
					Ospi_Nor_Ctx[Instance].InterfaceMode,
					MX25LM51245G_4BYTES_SIZE, pData, ReadAddr,
					Size) != MX25LM51245G_OK) {
				ret = BSP_ERROR_COMPONENT_FAILURE;
			} else {
				ret = BSP_ERROR_NONE;
			}
		} else {
			if (MX25LM51245G_ReadDTR(&hospi_nor[Instance], pData, ReadAddr,
					Size) != MX25LM51245G_OK) {
				ret = BSP_ERROR_COMPONENT_FAILURE;
			} else {
				ret = BSP_ERROR_NONE;
 ...
 * @retval BSP status
 */
int32_t BSP_OSPI_NOR_Write(uint32_t Instance, const uint8_t *pData,
	...
	if (Instance >= OSPI_NOR_INSTANCES_NUMBER) {
		...
		current_size = MX25LM51245G_PAGE_SIZE
				- (WriteAddr % MX25LM51245G_PAGE_SIZE );
		...
		do {
			/* Check if Flash busy ? */
			if (MX25LM51245G_AutoPollingMemReady(&hospi_nor[Instance],
					Ospi_Nor_Ctx[Instance].InterfaceMode,
					Ospi_Nor_Ctx[Instance].TransferRate) != MX25LM51245G_OK) {
				ret = BSP_ERROR_COMPONENT_FAILURE;
			}/* Enable write operations */
			else if (MX25LM51245G_WriteEnable(&hospi_nor[Instance],
					Ospi_Nor_Ctx[Instance].InterfaceMode,
					Ospi_Nor_Ctx[Instance].TransferRate) != MX25LM51245G_OK) {
				ret = BSP_ERROR_COMPONENT_FAILURE;
			} else {
				if (Ospi_Nor_Ctx[Instance].TransferRate
						== BSP_OSPI_NOR_STR_TRANSFER) {
					/* Issue page program command */
					if (MX25LM51245G_PageProgram(&hospi_nor[Instance],
							Ospi_Nor_Ctx[Instance].InterfaceMode,
							MX25LM51245G_4BYTES_SIZE, (uint8_t*) data_addr,
							current_addr, current_size) != MX25LM51245G_OK) {
						ret = BSP_ERROR_COMPONENT_FAILURE;
					}
				} else {
					/* Issue page program command */
					if (MX25LM51245G_PageProgramDTR(&hospi_nor[Instance],
							(uint8_t*) data_addr, current_addr,
				...
				if (ret == BSP_ERROR_NONE) {
					...
					if (MX25LM51245G_AutoPollingMemReady(&hospi_nor[Instance],
						...
						current_size =
								((current_addr + MX25LM51245G_PAGE_SIZE )
										> end_addr) ?
 ...
 * @retval BSP status
 */
int32_t BSP_OSPI_NOR_Erase_Block(uint32_t Instance, uint32_t BlockAddress,
	...
	if (Instance >= OSPI_NOR_INSTANCES_NUMBER) {
		...
		else if (MX25LM51245G_BlockErase(&hospi_nor[Instance],
				Ospi_Nor_Ctx[Instance].InterfaceMode,
...
int32_t BSP_OSPI_NOR_Erase_Chip(uint32_t Instance) {
	...
	if (Instance >= OSPI_NOR_INSTANCES_NUMBER) {
		...
		else if (MX25LM51245G_ChipErase(&hospi_nor[Instance],
				Ospi_Nor_Ctx[Instance].InterfaceMode,
				Ospi_Nor_Ctx[Instance].TransferRate) != MX25LM51245G_OK) {
			ret = BSP_ERROR_COMPONENT_FAILURE;
		} else {
...
int32_t BSP_OSPI_NOR_GetStatus(uint32_t Instance) {
	...
	if (Instance >= OSPI_NOR_INSTANCES_NUMBER) {
		...
		if (MX25LM51245G_ReadSecurityRegister(&hospi_nor[Instance],
				Ospi_Nor_Ctx[Instance].InterfaceMode,
				Ospi_Nor_Ctx[Instance].TransferRate, reg) != MX25LM51245G_OK) {
			ret = BSP_ERROR_COMPONENT_FAILURE;
		}/* Check the value of the register */
		else if ((reg[0] & (MX25LM51245G_SECR_P_FAIL | MX25LM51245G_SECR_E_FAIL))
				!= 0U) {
			ret = BSP_ERROR_COMPONENT_FAILURE;
		} else if ((reg[0] & (MX25LM51245G_SECR_PSB | MX25LM51245G_SECR_ESB))
				!= 0U) {
			ret = BSP_ERROR_OSPI_SUSPENDED;
		} else if (MX25LM51245G_ReadStatusRegister(&hospi_nor[Instance],
				Ospi_Nor_Ctx[Instance].InterfaceMode,
				Ospi_Nor_Ctx[Instance].TransferRate, reg) != MX25LM51245G_OK) {
			ret = BSP_ERROR_COMPONENT_FAILURE;
		}/* Check the value of the register */
...
int32_t BSP_OSPI_NOR_EnableMemoryMappedMode(uint32_t Instance) {
	...
	if (Instance >= OSPI_NOR_INSTANCES_NUMBER) {
		...
		if (Ospi_Nor_Ctx[Instance].TransferRate == BSP_OSPI_NOR_STR_TRANSFER) {
			if (MX25LM51245G_EnableMemoryMappedModeSTR(&hospi_nor[Instance],
					Ospi_Nor_Ctx[Instance].InterfaceMode,
...
int32_t BSP_OSPI_NOR_ReadID(uint32_t Instance, uint8_t *Id) {
	...
	if (Instance >= OSPI_NOR_INSTANCES_NUMBER) {
		ret = BSP_ERROR_WRONG_PARAM;
	} else if (MX25LM51245G_ReadID(&hospi_nor[Instance],
			Ospi_Nor_Ctx[Instance].InterfaceMode,
 ...
 * @retval BSP status
 */
int32_t BSP_OSPI_NOR_ConfigFlash(uint32_t Instance,
		...
		if (Ospi_Nor_Ctx[Instance].IsInitialized == OSPI_ACCESS_MMP) {
			...
			case BSP_OSPI_NOR_OPI_MODE : /* 8-8-8 commands */
				if ((Mode != BSP_OSPI_NOR_OPI_MODE )
						|| (Rate != Ospi_Nor_Ctx[Instance].TransferRate)) {
					/* Exit OPI mode */
					ret = OSPI_NOR_ExitOPIMode(Instance);

					if ((ret == BSP_ERROR_NONE)
							&& (Mode == BSP_OSPI_NOR_OPI_MODE )) {
...
int32_t BSP_OSPI_NOR_SuspendErase(uint32_t Instance) {
	...
	/* Check whether the device is busy (erase operation is in progress). */
		...
		ret = BSP_ERROR_COMPONENT_FAILURE;
	} else if (MX25LM51245G_Suspend(&hospi_nor[Instance],
			Ospi_Nor_Ctx[Instance].InterfaceMode,
			Ospi_Nor_Ctx[Instance].TransferRate) != MX25LM51245G_OK) {
		ret = BSP_ERROR_COMPONENT_FAILURE;
...
int32_t BSP_OSPI_NOR_ResumeErase(uint32_t Instance) {
	...
	/* Check whether the device is busy (erase operation is in progress). */
		...
		ret = BSP_ERROR_COMPONENT_FAILURE;
	} else if (MX25LM51245G_Resume(&hospi_nor[Instance],
			Ospi_Nor_Ctx[Instance].InterfaceMode,
			Ospi_Nor_Ctx[Instance].TransferRate) != MX25LM51245G_OK) {
		ret = BSP_ERROR_COMPONENT_FAILURE;
	}
...
int32_t BSP_OSPI_NOR_EnterDeepPowerDown(uint32_t Instance) {
	...
	if (Instance >= OSPI_NOR_INSTANCES_NUMBER) {
		ret = BSP_ERROR_WRONG_PARAM;
	} else if (MX25LM51245G_EnterPowerDown(&hospi_nor[Instance],
			Ospi_Nor_Ctx[Instance].InterfaceMode,
			Ospi_Nor_Ctx[Instance].TransferRate) != MX25LM51245G_OK) {
		ret = BSP_ERROR_COMPONENT_FAILURE;
	} else {
...
int32_t BSP_OSPI_NOR_LeaveDeepPowerDown(uint32_t Instance) {
	...
	if (Instance >= OSPI_NOR_INSTANCES_NUMBER) {
		ret = BSP_ERROR_WRONG_PARAM;
	} else if (MX25LM51245G_NoOperation(&hospi_nor[Instance],
			Ospi_Nor_Ctx[Instance].InterfaceMode,
			Ospi_Nor_Ctx[Instance].TransferRate) != MX25LM51245G_OK) {
		ret = BSP_ERROR_COMPONENT_FAILURE;
	} else {
...
static void OSPI_NOR_MspInit(XSPI_HandleTypeDef *hospi) {
	...
	HAL_GPIO_EnableHighSPeedLowVoltage(OSPI_NOR_CLK_GPIO_PORT,
			OSPI_NOR_CLK_PIN);
	...
	HAL_GPIO_EnableHighSPeedLowVoltage(OSPI_NOR_DQS_GPIO_PORT,
			OSPI_NOR_DQS_PIN);
...
static int32_t OSPI_NOR_ResetMemory(uint32_t Instance) {
	...
	BSP_OSPI_NOR_STR_TRANSFER) != MX25LM51245G_OK) {
		ret = BSP_ERROR_COMPONENT_FAILURE;
	} else if (MX25LM51245G_ResetMemory(&hospi_nor[Instance],
			BSP_OSPI_NOR_SPI_MODE,
			BSP_OSPI_NOR_STR_TRANSFER) != MX25LM51245G_OK) {
		ret = BSP_ERROR_COMPONENT_FAILURE;
	} else if (MX25LM51245G_ResetEnable(&hospi_nor[Instance],
...
static int32_t OSPI_NOR_EnterDOPIMode(uint32_t Instance) {
	...
	/* Enable write operations */
	if (MX25LM51245G_WriteEnable(&hospi_nor[Instance],
			Ospi_Nor_Ctx[Instance].InterfaceMode,
			Ospi_Nor_Ctx[Instance].TransferRate) != MX25LM51245G_OK) {
		ret = BSP_ERROR_COMPONENT_FAILURE;
	}
	/* Write Configuration register 2 (with new dummy cycles) */
	else if (MX25LM51245G_WriteCfg2Register(&hospi_nor[Instance],
			Ospi_Nor_Ctx[Instance].InterfaceMode,
	...
	/* Write Configuration register 2 (with Octal I/O SPI protocol) */
		...
		else if (MX25LM51245G_ReadCfg2Register(&hospi_nor[Instance],
				BSP_OSPI_NOR_OPI_MODE, BSP_OSPI_NOR_DTR_TRANSFER,
...
static int32_t OSPI_NOR_EnterSOPIMode(uint32_t Instance) {
	...
	/* Write Configuration register 2 (with Octal I/O SPI protocol) */
		...
		/* Check the configuration has been correctly done */
				...
				BSP_OSPI_NOR_OPI_MODE, BSP_OSPI_NOR_STR_TRANSFER,
				MX25LM51245G_CR2_REG1_ADDR, reg) != MX25LM51245G_OK) {
			ret = BSP_ERROR_COMPONENT_FAILURE;
...
static int32_t OSPI_NOR_ExitOPIMode(uint32_t Instance) {
	...
	if (MX25LM51245G_WriteEnable(&hospi_nor[Instance],
		...
		reg[0] = 0;
		reg[1] = 0;
		if (MX25LM51245G_WriteCfg2Register(&hospi_nor[Instance],
				Ospi_Nor_Ctx[Instance].InterfaceMode,
				Ospi_Nor_Ctx[Instance].TransferRate, MX25LM51245G_CR2_REG1_ADDR,
			...
			if (Ospi_Nor_Ctx[Instance].TransferRate == BSP_OSPI_NOR_DTR_TRANSFER) {
				...
				hospi_nor[Instance].Init.DelayHoldQuarterCycle =
						HAL_XSPI_DHQC_DISABLE;
				if (HAL_XSPI_Init(&hospi_nor[Instance]) != HAL_OK) {
					ret = BSP_ERROR_PERIPH_FAILURE;
				}
			}
			...
			if (ret == BSP_ERROR_NONE) {
				...
				/* Check the configuration has been correctly done */
						...
						BSP_OSPI_NOR_SPI_MODE, BSP_OSPI_NOR_STR_TRANSFER,
						MX25LM51245G_CR2_REG1_ADDR, reg) != MX25LM51245G_OK) {
					ret = BSP_ERROR_COMPONENT_FAILURE;
				} else if (reg[0] != 0U) {
					ret = BSP_ERROR_COMPONENT_FAILURE;
